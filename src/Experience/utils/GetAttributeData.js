...
const getAttributeData = (instances, width) => {
    const offesets = new Float32Array(instances * 3 ); // 3 values (x, y, z) per instances of grass balde
    const orientations = new Float32Array( instances * 4 ); // 4 values per quartanion for each grass blade
    const streches = new Float32Array(instances); // 1 value for each grass blade
    ...
    for (let i=0; i< instances; i++){
        ...
        orientations.push(quarternion_0.x, quarternion_0.y, quarternion_0.z, quarternion_0.w);

        //variation in height
        if(i < instances/3){
            streches.push(Math.random() * 1.8);
        }else{
            streches.push(Math.random());
        }
